...
router.get('/', auth, hasRole('professor'), async (req,res,next)=>{
  try{ const items = await Notification.find({user:req.user._id}).sort('-createdAt'); res.json(items); }
  catch(e){ next(e); }
});
router.post('/:id/read', auth, hasRole('professor'), async (req,res,next)=>{
...
router.post('/mark-all-read', auth, hasRole('professor'), async (req,res,next)=>{
  try{ await Notification.updateMany({user:req.user._id, read:false}, {$set:{read:true}}); res.json({ok:true}); }
  catch(e){ next(e); }
});
router.get('/unread-count', auth, hasRole('professor'), async (req,res,next)=>{
