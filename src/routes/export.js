...
router.get('/articles.pdf', async (req,res,next)=>{
  try{
    ...
    items.forEach((a,i)=>{
      ...
      doc.text(`   Professor: ${a.authorName||''}`);
      doc.text(`   Student: ${a.mentorName || a.mentorEmail || '-'}`);
...
router.get('/articles.csv', async (req,res,next)=>{
  try{
    ...
    const csv = createCsvWriter({ path: tmp, header:[
      {id:'title', title:'Title'},{id:'authorName', title:'Professor'},{id:'mentorEmail', title:'MentorEmail'},{id:'mentorName', title:'MentorName'},
